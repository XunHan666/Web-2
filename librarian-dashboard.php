...
/**
 * Librarian Dashboard
 * Only accessible to role_id = 2 (Librarian / Staff)
 */
...
$pending_requests_count     = mysqli_fetch_array(mysqli_query($db_connect, "SELECT COUNT(*) FROM requests WHERE status = 'pending' AND type = 'borrow_book'"))[0];

$staff_display_name   = $_SESSION['full_name'] ?? 'Librarian';
$staff_role_label     = $_SESSION['role_name'] ?? 'Librarian';
$hour = (int)date('H');
    ...
    <p style="color: var(--text-muted); font-size: 1rem; display: flex; align-items: center; gap: 0.5rem;">
        ...
        <span>
            ...
            <?php if ($pending_circulation_count > 0): ?>
                <strong style="color: var(--danger); font-weight: 700;"><?php echo $pending_circulation_count; ?> active transactions</strong>
            <?php else: ?>
...
<h2 style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-muted); margin-bottom: 1.25rem; font-weight: 700;">Today's Overview</h2>
<div class="stats-grid compact-stats" style="margin-bottom: 4rem;">
    ...
    <div class="stat-card" style="border-left: 4px solid var(--success);">
        <h3 style="color: var(--success);">Available</h3>
        <div class="value" style="color: var(--success);"><?php echo $ready_for_loan_count; ?></div>
        <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 5px;">Ready for loan</p>
    </div>
    <div class="stat-card" style="border-left: 4px solid #0ea5e9;">
        <h3 style="color: #0284c7;">Readers</h3>
        <div class="value" style="color: #0ea5e9;"><?php echo $total_registered_readers; ?></div>
        <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 5px;">Registered members</p>
    </div>
    <div class="stat-card <?php echo ($pending_circulation_count > 0) ? 'stat-card-danger' : ''; ?>" style="border-left: 4px solid var(--danger);">
        ...
        <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 5px;">Currently borrowed</p>
    </div>
    <div class="stat-card <?php echo ($pending_requests_count > 0) ? 'stat-card-danger' : ''; ?>" style="border-left: 4px solid #f59e0b;">
        <h3 style="color: #d97706;">Borrow Requests</h3>
        <div class="value" style="color: #f59e0b;"><?php echo $pending_requests_count; ?></div>
        <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 5px;">Awaiting approval</p>
    </div>
</div>

<h2 style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-muted); margin-bottom: 1.25rem; font-weight: 700;">Quick Actions</h2>
<div class="quick-actions-grid" style="margin-bottom: 2rem;">
    ...
    <a href="<?php echo BASE_URL; ?>loan/borrow.php" class="action-tile tile-secondary" title="Process a loan">
        <div class="tile-icon">
            <svg viewBox="0 0 24 24" width="28" height="28" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
        </div>
        <span>Borrow Book</span>
    </a>
    <a href="<?php echo BASE_URL; ?>loan/loans.php" class="action-tile tile-success" title="Process a return">
        <div class="tile-icon">
