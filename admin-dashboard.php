...
/**
 * Admin Dashboard
 * Only accessible to role_id = 1 (Admin)
 */
...
$total_accounts_res         = mysqli_query($db_connect, "SELECT COUNT(*) FROM accounts WHERE id != $admin_id");
$total_accounts             = mysqli_fetch_array($total_accounts_res)[0];
$pending_requests_count     = mysqli_fetch_array(mysqli_query($db_connect, "SELECT COUNT(*) FROM requests WHERE status = 'pending'"))[0];

$staff_display_name   = $_SESSION['full_name'] ?? 'Admin';
$staff_role_label     = $_SESSION['role_name'] ?? 'Administrator';
$hour = (int)date('H');
